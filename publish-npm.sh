...
echo -e "${YELLOW}Updating jfrog-native-ui version: ${GREEN}$1${YELLOW} in 'package.json'${NC}"
...
echo -e "${YELLOW}Building jfrog-native-ui:${NC}"
gulp build
...
select yn in "Yes" "No"; do
    case $yn in
        Yes ) echo -e "${YELLOW}Enter AUTH:${NC}"
            read authKey
            curl -H "X-JFrog-Art-Api:${authKey}" 'https://repo.jfrog.io/artifactory/ui/artifactactions/delete' --data-binary '{"repoKey":"npmjs-cache","path":"jfrog-native-ui/-"}' -H 'Content-Type:application/json'
        break;;
        No )
        break;;
    esac
